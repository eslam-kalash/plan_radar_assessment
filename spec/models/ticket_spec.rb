...
RSpec.describe Ticket, type: :model do
  ...
  describe 'scopes' do
    ...
    let!(:open_ticket) { create(:ticket, assigned_user: user, status_id: 1, due_date: DateTime.now.utc) }
    let!(:closed_ticket) { create(:ticket, assigned_user: user, status_id: 3, due_date: DateTime.now.utc) }
    let!(:ticket_due_soon) { create(:ticket, assigned_user: user, status_id: 1, due_date: (DateTime.now + 3.day).utc) }
    ...
    describe '.user_open_tickets' do
      it 'returns only tickets assigned to the user with open statuses' do
        ...
        expect(result).to match_array([open_ticket, ticket_due_soon])
      end
    ...
    describe '.due_within' do
      ...
      it 'excludes tickets outside the specified time range' do
        ...
        expect(result).not_to include(ticket_due_soon)
      end
    end
  end
end
